...
!!! abstract "この章について"
    この章では、スタナビのAI機能の使い方をまとめます。メニューの「**AIレポート**」から使う **AI リサーチレポート**（期間を指定してAIが運勢レポートを生成）を中心に、あわせて **LISAチャット**（AIと対話形式で鑑定）、および出生図から作る **解読レポート** の操作方法を説明します。「AIレポート」メニューは **Plus プラン以上** でご利用いただけます（LISAチャットは **Basic 以上**・画面右下のLISAボタンから）。
...
- クレジットの残高照会・購入は ARI マイページで行います。スタナビ上のボタンはその画面への入口です。
