...
シナストリーのグリッドタブでは、二人の相性を見る際に重要と思われる「**良いアスペクト**」「**悪いアスペクト**」を、**ARI 独自の解釈** に基づいて **3色** で色分けして表示しています（対象はオーブ5度以内）。相性をひと目で読み解くのに便利です。
...
- クロスアスペクトで表示するアスペクトの種類・オーブ・色は、[設定](settings.md)（または「表示設定」パネル）のアスペクト設定にある「**二重円**」タブの内容が使われます。
